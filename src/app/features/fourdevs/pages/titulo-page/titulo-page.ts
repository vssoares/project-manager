...
import { generateMany, generateTitulo } from '../../data-access/generators'

@Component({
  selector: 'app-titulo-page',
  imports: [FormsModule, GeneratorShell],
  template: `
    <app-generator-shell title="Gerador de título de eleitor" description="Gera títulos de eleitor válidos. Uso apenas para testes." [results]="results()" (generate)="onGenerate()">
...
export class TituloPage {
  ...
  onGenerate() {
    this.results.set(generateMany(this.count, () => generateTitulo(this.masked)))
  }
}
