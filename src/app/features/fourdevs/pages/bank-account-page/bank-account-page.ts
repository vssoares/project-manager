...
import { generateBankAccount, generateMany } from '../../data-access/generators'

@Component({
  selector: 'app-bank-account-page',
  imports: [FormsModule, GeneratorShell],
  template: `
    <app-generator-shell title="Gerador de conta bancária" description="Gera agência/conta fictícias de bancos brasileiros. Uso apenas para testes." [results]="results()" (generate)="onGenerate()">
...
export class BankAccountPage {
  ...
  onGenerate() {
    this.results.set(generateMany(this.count, () => generateBankAccount()))
  }
}
