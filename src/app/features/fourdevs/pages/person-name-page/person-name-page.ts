...
import { generateMany, generatePersonName } from '../../data-access/generators'

@Component({
  selector: 'app-person-name-page',
  imports: [FormsModule, GeneratorShell],
  template: `
    <app-generator-shell title="Gerador de nome" description="Gera nomes brasileiros fictícios para testes." [results]="results()" (generate)="onGenerate()">
...
export class PersonNamePage {
  count = 1
  readonly results = signal<string[]>([])
  onGenerate() {
    this.results.set(generateMany(this.count, () => generatePersonName()))
  }
}
