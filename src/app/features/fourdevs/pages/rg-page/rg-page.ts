...
import { generateMany, generateRg } from '../../data-access/generators'

@Component({
  selector: 'app-rg-page',
  imports: [FormsModule, GeneratorShell],
  template: `
    <app-generator-shell title="Gerador de RG" description="Gera RG no padrão SP (com dígito verificador). Uso apenas para testes." [results]="results()" (generate)="onGenerate()">
...
export class RgPage {
  ...
  onGenerate() {
    this.results.set(generateMany(this.count, () => generateRg(this.masked)))
  }
}
